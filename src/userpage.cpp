...
inline bool IsActiveConsoleSession()
{
    DWORD dwCurrentSession = 0;
    if (ProcessIdToSessionId(GetCurrentProcessId(), &dwCurrentSession))
    {
        return WTSGetActiveConsoleSessionId() == dwCurrentSession;
    }
    return false;
}
            ...
            if (j < nSessions)
            {
                ...
                case WTSShadow:
                    ...
                    hr = pUserInfo->SetData(
                                        (pszClientName == NULL) ? (LPTSTR)TEXT("") : pszClientName,
        ...
        case IDM_CONNECT:
            {
                ...
                for( ;; )
                {
                    ...
                    fRet = WinStationConnectW(hServer, pUser->m_dwSessionId, LOGONID_CURRENT, szPassword, TRUE);
                    if( fRet )
...
void Shadow(HWND hwnd, CUserInfo * pUser)
{
    ...
    if(!WinStationQueryInformationW(hServer,
                                ...
                                &WSConfig, sizeof(WINSTATIONCONFIGW),
                                &ReturnLength ) ) 
    {
